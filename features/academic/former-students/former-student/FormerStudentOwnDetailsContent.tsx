...
export function FormerStudentOwnDetailsContent({
	...
	const fields = useMemo<EntityPageField[]>(() => {
		const baseFields: EntityPageField[] = [
			...
			{
				id: "progress",
				label: t("academic.studentPage.table.columns.progress"),
				value: `${formerStudent.counterpartHours.progress.toFixed(2)}%`,
			},
			{
				id: "concluded",
				label: t("academic.studentPage.table.columns.concluded"),
				value: (
					<Badge
		...
		return [
			{
				...
				label: t("academic.studentPage.dialog.fields.academicRegistration"),
