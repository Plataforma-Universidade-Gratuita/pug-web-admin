...
import type { AttendanceCreateRequest, AttendanceResponse, AttendanceValidateRequest } from "@/types/api";
...
export async function list(projectId?: string, studentId?: string): Promise<AttendanceResponse[]> {
	const params = new URLSearchParams();
	if (projectId) params.set("projectId", projectId);
	if (studentId) params.set("studentId", studentId);
	const search = params.toString();
	return webFetch(`${BASE}${search ? `?${search}` : ""}`, z.array(AttendanceResponseSchema));
}

export async function create(body: AttendanceCreateRequest): Promise<AttendanceResponse> {
...
export async function validate(id: string, body: AttendanceValidateRequest): Promise<AttendanceResponse> {
