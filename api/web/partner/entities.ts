...
export async function list(q?: string, cityId?: string): Promise<EntityResponse[]> {
	const params = new URLSearchParams();
	if (q) params.set("q", q);
	if (cityId) params.set("cityId", cityId);
	const search = params.toString();
	return webFetch(`${BASE}${search ? `?${search}` : ""}`, z.array(EntityResponseSchema));
}
...
export async function create(body: EntityCreateRequest): Promise<EntityResponse> {
...
export async function update(id: string, body: EntityUpdateRequest): Promise<EntityResponse> {
	return webFetch(`${BASE}/${id}`, EntityResponseSchema, {
		method: "PUT",
