...
export async function get(id: string): Promise<SchoolResponse> {
	return webFetch(
		`${WEB_API_ROUTE_BASES.academic.schools}/${id}`,
		SchoolResponseSchema,
	);
}
...
export async function list(q?: string): Promise<SchoolResponse[]> {
	...
	return webFetch(
		`${WEB_API_ROUTE_BASES.academic.schools}${search}`,
...
export async function create(
	...
	return webFetch(
		`${WEB_API_ROUTE_BASES.academic.schools}`,
		SchoolResponseSchema,
		{
			method: "POST",
...
export async function remove(id: string): Promise<void> {
	return webVoid(`${WEB_API_ROUTE_BASES.academic.schools}/${id}`, {
		method: "DELETE",
	});
}
